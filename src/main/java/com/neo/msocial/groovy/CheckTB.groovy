...
*/
  ...
  """;
                    String ret = new Activation().soapCall(context.get("dataflow_param:sqlmodule"), request);
                    if (ret.equals("1")) {
                        if (sendSmsForSharing && context.get("channel").equals("SMS")) {
  ...
  """;
                ...
                String ret = new Activation().soapCall(context.get("dataflow_param:sqlmodule"), request);
                if (Integer.parseInt(getValueFromKey(new Activation().parseXMLtext(ret, "//*[local-name() = 'return']"), "TOTAL_SMS")) >= maxSms) {
