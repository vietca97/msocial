...
<vms:P12>$scriptShopId</vms:P12>      
    ...
    def getValueFromKeyMultiRecords = { String body, String rootTag, String tagName, String key, String value ->
        String ret = parseXml.getValueFromKey(body, rootTag, tagName, key, value);
	...
	""";
        try {
            String transId = activation.parseXMLtext(activation.soapCall(context.get("dataflow_param:sqlmodule"), request), "//*[local-name() = 'return']");
            System.out.println("---ret:" + transId + "----xxsxs-");
            transId = getValueFromKey(transId, "TRANSACTION_ID");
//new XmlSlurper().parseText(transId).record.TRANSACTION_ID.text();
	...
	""";
		...
		""";
            try {
                ...
                String retval = activation.parseXMLtext(activation.soapCall(context.get("dataflow_param:sqlmodule"), request), "//*[local-name() = 'return']");
	...
	""";
        ...
        String cycle = "";
        try {
            pckInfo = activation.parseXMLtext(activation.soapCall(context.get("dataflow_param:utilmodule"), request), "//*[local-name() = 'return']");
            ...
            if (usedPckPrice.equals("M120") && (packageCode.equals("M120") || packageCode.equals("KM120"))) {
                retval[0] = "10"; retval[1] = pckNeedCheck; retval[2] = usedPckPrice; retval[3] = pckNeedCheck;
                retval[4] = usedPckPrice; retval[5] = capacity; retval[6] = cycle;
                return retval;
            }
            ...
            //gia goi dang ky > gia goi thue bao dang su dung
                ...
                retval[0] = "2"; retval[1] = packageCode; retval[2] = registerPckPrice; retval[3] = pckNeedCheck;
                retval[4] = usedPckPrice; retval[5] = capacity; retval[6] = cycle; retval[7] = capacityPckNeedCheck;
                retval[8] = cyclePckNeedCheck;
            } else if (registerPckPrice < usedPckPrice) {
                retval[0] = "3"; retval[1] = packageCode; retval[2] = registerPckPrice; retval[3] = pckNeedCheck;
                retval[4] = usedPckPrice; retval[5] = capacity; retval[6] = cycle; retval[7] = capacityPckNeedCheck;
                retval[8] = cyclePckNeedCheck;
            } else {
                retval[0] = "1";
    ...
    def checkPkgDaiky = { String msisdn, String pkg, String channelName ->
        try {
			...
			""";
            String result = activation.parseXMLtext(activation.soapCall(utilUrl, request), "//*[local-name() = 'return']");
            return result;
        } catch (Exception e) {
            return "-1|" + e.getMessage();
        }
    ...
    boolean processBusiness(String $ma_chi_nhanh, String $script_shop_id, String msisdn,
                            String serviceid, String price, List<Soap34> $soap_34_extract1,
                            List<Soap8> $soap_8_extract1, String packagecode,
                            String channel, String $partner_id, String $sharing_key_id, String $channel_id, String $agent_id, sharingkey) {
        //System.out.println("AAAAAAAAAABCCCCCCCCCC:"+context.get("check_mi_fc_stxatus"));
...
//System.out.println("BILLING_FLAG:"+billing_flag);
        ...
        try {
            ...
            if (context.get("SERVICE_KEY").equals("MOBILEINTERNET") || context.get("SERVICE_KEY").equals("FASTCONNECT")) {
                ...
                if (info != null && info[0] == "0") {
					...
					""";
                            ...
                            String ret = activation.parseXMLtext(activation.soapCall(context.get("dataflow_param:sqlmodule"), request), "//*[local-name() = 'return']");
                            System.out.println("---------DAILV_CLD:" + request);
			...
			""";
                    ...
                    String infosFunring = "";
                    try {
                        infosFunring = activation.parseXMLtext(activation.soapCall(context.get("dataflow_param:utilmodule"), request), "//*[local-name() = 'return']");
