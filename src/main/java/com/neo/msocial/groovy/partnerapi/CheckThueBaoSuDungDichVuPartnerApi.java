...
@Component
public class CheckThueBaoSuDungDichVuPartnerApi {
    @Autowired
...
//                                else cmdStatus = "UNUSE";
                            ...
                            if (cmdStatus.equals("INUSE")) {
                                ...
                                if (context.get("SEND_SMS").equals("1")) {
                                    String mt = utilServices.getValueFromKeySOAP8($soap_8_extract1,"SERVICE_IN_USE").replaceAll("\\{MSISDN}", String.valueOf(msisdn)).replaceAll("\\{SERVICE_KEY}", context.get("SERVICE_KEY_SMS"));
