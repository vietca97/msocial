...
@Component
public class CheckChongLoiDung {
    @Autowired
...
//kiem tra chinh sach chong loi dung
        ...
        try {
            ...
            try {
                ...
                try{
                    String policy = context.get("SERVICE_KEY")+"_WHITE_LIST";

                    str_soap = new StringBuilder();
                    str_soap.append("<soapenv:Envelope xmlns:soapenv=\"http://schemas.xmlsoap.org/soap/envelope/\" xmlns:vms=\"http://vms.neo\">");
                    str_soap.append("<soapenv:Header/><soapenv:Body>");
                    str_soap.append("<vms:value>");
                    str_soap.append("<vms:Service>checkwhitelist</vms:Service>");
                    ...
                    str_soap.append("<vms:P2>").append(serviceid).append("</vms:P2>");
                    str_soap.append("<vms:P3>").append(policy).append("</vms:P3>");
                    str_soap.append("<vms:P4>").append(sharingkey).append("</vms:P4>");
                    str_soap.append("</vms:value></soapenv:Body></soapenv:Envelope>");
                    ret = new Activation().parseXMLtext(new Activation().soapCall(context.get("dataflow_param:sqlmodule"),str_soap.toString()),"//*[local-name() = 'return']");
                    System.out.println("AAAAAAAAAAA_DAILV:"+str_soap.toString());
                    System.out.println("AAAAAAAAAAA_DAILV:"+ret);
                    try{

                        if(Integer.parseInt(ret)<=0){
                            context.put("ErrorCodeAPI","221");context.put("ErrorDescAPI","Thong bao ban khong duoc gioi thieu goi cuoc nay cho thue bao");
                            stepResult = false;
                        }
