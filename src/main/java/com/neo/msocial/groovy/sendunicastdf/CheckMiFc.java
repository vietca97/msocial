...
public class CheckMiFc {
    ...
    @Autowired
    private GenericsRequest<ServicePackageDetailDto> servicePackageDetailDtoGenericsRequest;
...
//        int tempAcceptId = Integer.parseInt(getValueFromKey(ret, "ACCEPT_ID"));
            ...
            try {
                ...
                String retval = new Activation().parseXMLtext(new Activation().soapCall(context.get("dataflow_param:sqlmodule"), str_soap.toString()), "//*[local-name() = 'return']");
...
//check thue bao huy
        ...
        System.out.println("mi_status_info:" + mi_status_info);
        System.out.println("mi_huy_info:" + mi_huy_info);
        System.out.println("PACKAGE_CYCLE:" + context.get("PACKAGE_CYCLE"));
...
//        }
        ...
        try {
            if (!context.get("SERVICE_KEY").equals("MOBILEINTERNET") && !context.get("SERVICE_KEY").equals("FASTCONNECT")) {
                ...
                if (context.get("NEED_CHECK_SERVICE").equals("0")) return true;
                try {
                    pckCycle = Integer.parseInt(context.get("PACKAGE_CYCLE"));
                    System.out.println("pckCYCLE:" + pckCycle);
                } catch (Exception ex) {
                    ex.printStackTrace();
                    pckCycle = -1;
                    System.out.println("pckCYCLE:" + pckCycle);
                ...
                if (pckCycle == 30) {
                    if (!packageCode.startsWith("KM") && !packageCode.startsWith("MAX")) {
                        if (mi_status_info == "" || mi_status_info.indexOf("DATA_WEB_SUCC") < 0) {
                            ...
                            if (packageCode.equals("8COM") || packageCode.equals("ZING")) iscontinue = false;
                ...
                if (context.get("SERVICE_KEY").equals("FASTCONNECT")) {
                    ...
                    String msisdnTmp = msisdn;
                    if (msisdn.startsWith("84")) msisdnTmp = msisdn.substring(2);
                    StringBuilder request = new StringBuilder();
                    request.append("<soapenv:Envelope xmlns:soapenv=\"http://schemas.xmlsoap.org/soap/envelope/\" xmlns:vms=\"http://vms.neo\">");
                    request.append("<soapenv:Header/><soapenv:Body>");
                    request.append("<vms:sendCommand>");
                    ...
                    try {
                        ...
                        logSql(msisdn, "-1", "CHECK_PROFILE_FC_REQUEST", "5", "CHECK_PROFILE_FC_REQUEST", request.toString(), profile, context.get("ErrorCodeAPI") + "|" + context.get("ErrorDescAPI"), startTime, new SimpleDateFormat("yyyyMMddHHmmss").format(new Date()), "sharingkey= " + sharingkey + ",serviceid= " + serviceid + ",packagecode=" + packagecode + ",channel=" + channel, script_shop_id);
                    } catch (Exception ex) {
                        System.out.println(ex.getMessage());
                        context.put("ErrorCodeAPI", "-1");
                        context.put("ErrorDescAPI", "Loi khi tra cuu Profile thue bao FC");
...
//  ["FCZ:0","FCT2:0","TTN:0","FCPV:0","TSFC:1","CFC:0","TSFB:1","FCDN:0"];
                    ...
                    if (!foundFC) {
                        ...
                        context.put("ErrorCodeAPI", "27");
                        context.put("ErrorDescAPI", "Thue bao khong phai Fastconnect");
                        return false;
                    }
                    if (isTT) {
                        ...
                        try {
                            ...
                            logSql(msisdn, "-1", "CHECK_STARTDATE_FC_REQUEST", "6", "CHECK_STARTDATE_FC_REQUEST", str_soap.toString(), startDate, context.get("ErrorCodeAPI") + "|" + context.get("ErrorDescAPI"), startTime, new SimpleDateFormat("yyyyMMddHHmmss").format(new Date()), "sharingkey= " + sharingkey + ",serviceid= " + serviceid + ",packagecode=" + packagecode + ",channel=" + channel, script_shop_id);
                        } catch (Exception ex) {
                            System.out.println(ex.getMessage());
                            context.put("ErrorCodeAPI", "-1");
                ...
                if (packageCode.equals("KM1") || packageCode.equals("KM2") || packageCode.equals("KM3") || packageCode.equals("KM4") || packageCode.equals("KM6") || packageCode.equals("KM7") || packageCode.equals("KM8")) {
                    ...
                    if (info[0].equals("2")) {
                        ...
                        if(context.get("NEED_CHECK_SERVICE").equals("0")){
                        ...
                        context.put("ErrorCodeAPI", "-1");
                        context.put("ErrorDescAPI", "Loi khong tra cuu duoc thong tin.");
                        //logSql(msisdn, "-1", "CHECK_THUE_BAO_HUY_DICH_VU_MI_FC", "7", "CHECK_THUE_BAO_HUY_DICH_VU_MI_FC", context.get("checkhuy_cmdRequest_with_value") + "|stepResult=" + stepResult, context.get("checkhuy_cmdresponse_with_value") + "|stepResult=" + stepResult, context.get("ErrorCodeAPI") + "|" + context.get("ErrorDescAPI"), startTime, new SimpleDateFormat("yyyyMMddHHmmss").format(new Date()), "sharingkey= " + sharingkey + ",serviceid= " + serviceid + ",packagecode=" + packagecode + ",channel=" + channel + ",dataflow:registerService", script_shop_id);
